...
const EuConhecoEssa = () => {
  ...
  const { campos, getCampoById } = useCampoData();
  ...
  const carregarColetas = async () => {
    try {
      ...
      const response = getColetasOutrosUsuarios(user_id, campos, projetos);
      

      if (response.status === 200 && response.data) {
        // const coletasFiltradas = response.data.find((c) => {

        //   const campo = getCampoById(c.campo_id);
        //   if (campo.usuario_id === user_id) {
